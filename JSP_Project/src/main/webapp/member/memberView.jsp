...
<title>Insert title here</title>
  ...
  <script src="../js/member.js"></script> <!-- 유효성검사 js에게 시킴 -->
</head>
<%
...
<body>
	...
	/ <a href="logout.jsp">로그아웃</a>
		...
		<form action="memberUpdatePro.jsp" method="post" id="frm">
			<input type="hidden" name="userid" id="userid" value="<%=member.getUserid()%>"> <!-- 아이디 히든으로 들고온다. -->
			...
			<div class="form-group">
				<label for="email">Email:</label> <input type="text"
					class="form-control" id="email" name="email" value="<%=member.getEmail()%>">
			</div>

			<div class="form-group">
				<label for="phon">Phone:</label> <input type="text"
			...
			<script> // 일반회원은 일반회원 체크, 관리자는 관리자 체크되도록.
